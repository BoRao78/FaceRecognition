...
train_label_data = train_label(:,1:train_num * class_num);
test_label_data = test_label(:,1:test_num * class_num);
...
train_label_data = train_label_data(:,(ID(1:k*m1/80)));
ID = randperm(m2);
test_x = test_x(ID(1:k*m2/80),:);
test_label_data = test_label_data(:,(ID(1:k*m2/80)));
...
train_feature_data = dbnout(dbn,train_x,100); % Computing train feature using RBM, Gibbs Sampling number =100, HX
test_feature_data = dbnout(dbn,test_x,100); % Computing test feature using RBM, Gibbs Sampling number =100, HX
